...
--모아서 통계를 내준다.
--예로 SCORE테이블에 CNO을 GROUP BY 절에 작성하면
...
--NVL로 NULL값을 처리한 COMM값으로 합계를 내야한다.
SELECT DNO
     , SUM(NVL(COMM, 0))
    FROM EMP
    GROUP BY DNO;
...
--학과별 평점의 최고평균
--학과별 평점의 평균을 가져오는 쿼리 만들어진 가상 테이블의 전체 데이터를 사용해서 통계를 내는거니까
--별도의 GROUP BY는 필요없다.
SELECT MAX(AVG(AVR))
    FROM STUDENT
    GROUP BY MAJOR;
