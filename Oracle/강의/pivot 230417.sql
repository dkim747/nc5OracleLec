...
--GROUPING_ID
--GROUPING의 결과를 2진수로 인식하여 10진수로 변환한 값을 표시
SELECT DNO
     , JOB
     , MAX(SAL)
     , SUM(SAL)
...
--학과별 학년별 평점의 최대값 => 학과 컬럼으로 변경되는 쿼리
--PIVOT의 메인 FROM절은 사용할 컬럼만 가져올수 있도록 서브쿼리로
--테이블을 재구성 하던가 사용할 컬럼만 가지고 있는 테이블을 CREATE TABLE로 새로 만들어서 사용
SELECT *
    FROM (
            SELECT MAJOR
                 , SYEAR
    ...
    --데이터들이 컬럼이 되면서컬럼은 중복으로 존재할 수 없기때문에
    --자동으로 그룹화 된다.
    PIVOT(MAX(AVR)
        FOR MAJOR IN (
                        '화학' AS 화학,
...
SELECT *
    FROM (
            SELECT AVG(DECODE(CNAME, '일반화학', RESULT)) AS 일반화학
                 , AVG(DECODE(CNAME, '유기화학', RESULT)) AS 유기화학
                 , AVG(DECODE(CNAME, '물리화학', RESULT)) AS 물리화학
                FROM SCORE
                NATURAL JOIN COURSE
        )
    UNPIVOT (
        RESAVG FOR CNAME IN (일반화학, 유기화학, 물리화학)    
    );
